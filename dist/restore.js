import { g as getDefaultExportFromCjs, e as error, a as getCacheProvider, b as getInput, c as exportVariable, C as CacheConfig, i as info, d as cleanTargetDir, r as reportError, s as setOutput } from './cleanup-BtFCJ68u.js';
import * as fs from 'fs';
import { r as requireForm_data, a as requireDist, b as requireFollowRedirects } from './index-mOt-pWPW.js';
...
class AxiosTransformStream extends stream__default.Transform {
  constructor(options) {
    options = utils$1.toFlatObject(
      options,
      {
...
const textEncoder = typeof TextEncoder === 'function' ? new TextEncoder() : new util__default.TextEncoder();
...
class ZlibHeaderTransformStream extends stream__default.Transform {
    ...
    if (authoritySessions) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (
          ...
          util__default.isDeepStrictEqual(sessionOptions, options)
        ) {
  ...
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve$1, reject, onDone) {
      ...
      onDone((response, isRejected) => {
        ...
        if (data instanceof stream__default.Readable || data instanceof stream__default.Duplex) {
          const offListeners = stream__default.finished(data, () => {
            offListeners();
            onFinished();
          });
        } else {
      ...
      if (protocol === 'data:') {
        ...
        if (responseType === 'text') {
          ...
          convertedData = stream__default.Readable.from(convertedData);
        }
      ...
      if (utils$1.isSpecCompliantForm(data)) {
        ...
        if (!headers.hasContentLength()) {
          try {
            const knownLength = await util__default.promisify(data.getLength).call(data);
        ...
        data = stream__default.Readable.from(readBlob(data));
      ...
      if (data && (onUploadProgress || maxUploadRate)) {
        if (!utils$1.isStream(data)) {
          data = stream__default.Readable.from(data, { objectMode: false });
        }

        data = stream__default.pipeline(
          [
            data,
      ...
      req = transport.request(options, function handleResponse(res) {
        ...
        responseStream = streams.length > 1 ? stream__default.pipeline(streams, utils$1.noop) : streams[0];
        ...
        if (responseType === 'stream') {
          ...
          if (maxContentLength > -1) {
            ...
            responseStream = stream__default.Readable.from(enforceMaxContentLength(), {
              objectMode: false,
            });
          }
      ...
      if (utils$1.isStream(data)) {
        ...
        if (maxBodyLength > -1 && !transportEnforcesMaxBodyLength) {
          ...
          let bytesSent = 0;
          uploadStream = stream__default.pipeline(
            [
              data,
              new stream__default.Transform({
